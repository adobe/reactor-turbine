...
const companyId = process.env.RSDK_ADOBE_REACTOR_COMPANY_ID;
...
async function ensureExtensionInstalled({
  ...
  if (!extension) {
    ...
    console.log(`✅ "${extensionPackageName}" Extension created for property`);
  } else {
 ...
 * @returns {Promise<Object>} The created Rule Component
 */
async function createBrowserCondition({ propertyId, coreExtensionId, ruleId }) {
  return await createRuleComponent({
    propertyId,
    extensionId: coreExtensionId,
    ruleId,
...
async function prepareNewPropertyForDelegates({
  ...
  console.log(`✅ created property (${propertyId}) with name ${propertyName}`);
